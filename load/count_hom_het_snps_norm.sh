...
# The script was modified in Dec 2023 to acount for different sampling. I will take 4 tetraploids and 8 diploids to have the same number of chromosomes.
...
count () {
	...
	# I was thinking about QD but maybe I will use again some hard filter 8x 
        ...
        then            
			...
			grep PSC ${1/.gz.snpeff.ann.vcf/stats} | grep -v "#" | sort -r -k 10 | head -n 8 | cut -f 3 > ${1/.gz.snpeff.ann.vcf/samples} # get list of 6 samples with the highest coverage
        		...
        		dp=`grep PSC ${1/.gz.snpeff.ann.vcf/8x.subset.stats} | grep -v "#" | sort -r -k 10 | head -n 8 | awk '{s+=$10}END{print s/NR}'` # depth of subset vcf
			...
			bgzip $vcf
			tabix -p vcf ${vcf}.gz
                        bcftools view --max-ac 8 ${vcf}.gz > ${vcf/8x.subset./minor.}  #here consider only sites with minor allele, remove ac>6 it depends on number of indivs you select at line 36
			...
			WEAK_tot_minor=`grep  MODIFIER $vcf | grep -E -v "HIGH|MODERATE|LOW" | wc -l`
                        ...
                        MISSENSE_het=`grep missense_variant $vcf |cut -f 10-18|grep -E -o "0/1" | wc -l` # get heterozygot genotypes only and count them
                        SILENT_het=`grep synonymous_variant $vcf |grep -E -v "HIGH|MODERATE" |cut -f 10-18|grep -E -o "0/1" | wc -l`
                        MISSENSE_hom=`grep missense_variant $vcf |cut -f 10-18|grep -E -o "1/1" | wc -l` # get homozygous genotypes of given category and count them
                        SILENT_hom=`grep synonymous_variant $vcf|grep -E -v "HIGH|MODERATE"  |cut -f 10-18|grep -E -o "1/1" | wc -l`
        ...
        then
			...
			grep PSC ${1/.gz.snpeff.ann.vcf/stats} | grep -v "#" | sort -r -k 10 | head -n 4 | cut -f 3 > ${1/.gz.snpeff.ann.vcf/samples} # get list of 6 samples with the highest coverage
                        bcftools view -S ${1/.gz.snpeff.ann.vcf/samples} ${1/..gz./.8x.} | bcftools view --min-ac 1 > ${1/.gz.snpeff.ann.vcf/8x.subset.ann.vcf} # subset the vcf for the selection of best samples
                        vcf=${1/.gz.snpeff.ann.vcf/8x.subset.ann.vcf}

                        bcftools stats -s - $vcf > ${1/.gz.snpeff.ann.vcf/8x.subset.stats}
                        dp_raw=`grep PSC ${1/.gz.snpeff.ann.vcf/stats} | grep -v "#" | sort -r -k 10 | head -n 4 | awk '{s+=$10}END{print s/NR}'` # depth of the input vcf
                        dp=`grep PSC ${1/.gz.snpeff.ann.vcf/8x.subset.stats} | grep -v "#" | sort -r -k 10 | head -n 4 | awk '{s+=$10}END{print s/NR}'` # depth of subset vcf
			...
			bgzip $vcf
                        tabix -p vcf ${vcf}.gz
                        bcftools view --max-ac 8 ${vcf}.gz > ${vcf/8x.subset./minor.} # here consider only sites with minor allele, remove ac>12
                        vcf=${vcf/8x.subset./minor.}
			...
			WEAK_tot_minor=`grep  MODIFIER $vcf | grep -E -v "MISSENSE|NONSENSE" | wc -l`
                        ...
                        MISSENSE_het=`grep missense_variant $vcf |cut -f 10-14|grep -E -o "0/0/0/1|0/1/1/1|0/0/1/1" | wc -l` #  it counts all dosages
                        MISSENSE_het1=`grep missense_variant $vcf |cut -f 10-14|grep -E -o "0/0/0/1" | wc -l` #  dosage1
                        MISSENSE_het2=`grep missense_variant $vcf |cut -f 10-14|grep -E -o "0/0/1/1" | wc -l` #  dosage2
                        MISSENSE_het3=`grep missense_variant $vcf |cut -f 10-14|grep -E -o "0/1/1/1" | wc -l` #  dosage3
                        SILENT_het=`grep synonymous_variant $vcf |grep -E -v "HIGH|MODERATE" |cut -f 10-14|grep -E -o "0/0/0/1|0/1/1/1|0/0/1/1" | wc -l`
                        SILENT_het1=`grep synonymous_variant $vcf |grep -E -v "HIGH|MODERATE" |cut -f 10-14|grep -E -o "0/0/0/1" | wc -l`
                        SILENT_het2=`grep synonymous_variant $vcf |grep -E -v "HIGH|MODERATE" |cut -f 10-14|grep -E -o "0/0/1/1" | wc -l`
                        SILENT_het3=`grep synonymous_variant $vcf |grep -E -v "HIGH|MODERATE" |cut -f 10-14|grep -E -o "0/1/1/1" | wc -l`
                        MISSENSE_hom=`grep missense_variant $vcf |cut -f 10-14|grep -E -o "1/1/1/1" | wc -l`
                        SILENT_hom=`grep synonymous_variant $vcf |grep -E -v "HIGH|MODERATE" |cut -f 10-14|grep -E -o "1/1/1/1" | wc -l`
...
find $outdir -mindepth 1 -maxdepth 1 -type d -print0 | parallel -0 -j $nt "tar -cvf {}.tar {} && mv {}.tar $wd/$outdir" 
...
find $outdir -type f -name '*.tar' -print0 | parallel -0 -j $nt "tar -xvf {} && rm {}"
