...
# In this version of script modified in Dec 2023 I will select 4 tetraploid individuals and 8 diploid individuals per population to sample the same number of chromosomes.


nt='44'
wd=${PBS_O_WORKDIR}
outdir=gen.counts.norm
   	...
   	if [[ $ploidy -eq 1 ]]
		...
		if [[ $nind -ge 8 ]]
	        then
        		...
        		grep PSC ${1/.gz.snpeff.ann.vcf/stats} | grep -v "#" | sort -r -k 10 | head -n 8 | cut -f 3 > ${1/.gz.snpeff.ann.vcf/samples} # get list of 8 samples with the highest coverage
			...
			bcftools stats -s - $vcf > ${1/.gz.snpeff.ann.vcf/shortind.subset.stats}
        		...
        		dp_raw=`grep PSC ${1/.gz.snpeff.ann.vcf/stats} | grep -v "#" | sort -r -k 10 | head -n 8 | awk '{s+=$10}END{print s/NR}'` # depth of the input vcf
        		dp=`grep PSC ${1/.gz.snpeff.ann.vcf/shortind.subset.stats} | grep -v "#" | sort -r -k 10 | head -n 8 | awk '{s+=$10}END{print s/NR}'` # depth of subset vcf
			...
			bgzip $vcf
			tabix -p vcf ${vcf}.gz
                        bcftools view --max-ac 8 ${vcf}.gz > ${vcf/shortind.subset./minor.}  #here consider only sites with minor allele, remove ac>6 it depends on number of indivs you select at line 36
			...
			INTRON_tot_minor=`grep  intron_variant $vcf | grep -E -v "HIGH|MODERATE" | wc -l`
                        ...
                        HIGH_het=`grep HIGH $vcf |cut -f 10-18|grep -E -o "0/1" | wc -l` # get heterozygot genotypes only and count them
                        MODIF_het=`grep MODIFIER $vcf |grep -E -v "HIGH|MODERATE|LOW" |cut -f 10-18|grep -E -o "0/1" | wc -l`
                        HIGH_hom=`grep HIGH $vcf |cut -f 10-18|grep -E -o "1/1" | wc -l` # get homozygous genotypes of given category and count them
                        MODIF_hom=`grep MODIFIER $vcf|grep -E -v "HIGH|MODERATE|LOW"  |cut -f 10-18|grep -E -o "1/1" | wc -l`
			...
			HIGH_tot_singl=`grep -c HIGH $vcf`
                ...
                printf "%s\t%s\n" $pop "less than 8 individuals"
		fi
	elif [[ $ploidy -eq 3 ]]
	then
                if [[ $nind -ge 4 ]]
                then
			grep PSC ${1/.gz.snpeff.ann.vcf/stats} | grep -v "#" | sort -r -k 10 | head -n 4 | cut -f 3 > ${1/.gz.snpeff.ann.vcf/samples} # get list of 8 samples with the highest coverage
                        bcftools view -S ${1/.gz.snpeff.ann.vcf/samples} ${1/..gz./.shortind.} | bcftools view --min-ac 1 > ${1/.gz.snpeff.ann.vcf/shortind.subset.ann.vcf} # subset the vcf for the selection of best samples
                        vcf=${1/.gz.snpeff.ann.vcf/shortind.subset.ann.vcf}
                        bcftools stats -s - $vcf > ${1/.gz.snpeff.ann.vcf/shortind.subset.stats}
                        pop=`basename $1 | sed 's/..gz.snpeff.ann.vcf//g'`
                        dp_raw=`grep PSC ${1/.gz.snpeff.ann.vcf/stats} | grep -v "#" | sort -r -k 10 | head -n 4 | awk '{s+=$10}END{print s/NR}'` # depth of the input vcf
                        dp=`grep PSC ${1/.gz.snpeff.ann.vcf/shortind.subset.stats} | grep -v "#" | sort -r -k 10 | head -n 4 | awk '{s+=$10}END{print s/NR}'` # depth of subset vcf
                        sites_tot=`grep -v "#" $vcf | wc -l` # number of sites in subset vcf
                        HIGH_tot=`grep -c HIGH $vcf` # 90% of high effect indels are frameshift mutations
                        MODERATE_tot=`grep MODERATE $vcf| grep -v HIGH| wc -l` # this category is mainly indels multiple of three
                        MODIF_tot=`grep  MODIFIER $vcf| grep -E -v "HIGH|MODERATE|LOW" | wc -l` #mainly indels in non-coding regions
			...
			bgzip $vcf
                        tabix -p vcf ${vcf}.gz
                        bcftools view --max-ac 8 ${vcf}.gz > ${vcf/shortind.subset./minor.} # here consider only sites with minor allele, remove ac>8
                        vcf=${vcf/shortind.subset./minor.}
			...
			INTRON_tot_minor=`grep  intron_variant $vcf | grep -E -v "HIGH|MODERATE" | wc -l`
                        ...
                        HIGH_het=`grep HIGH $vcf |cut -f 10-14|grep -E -o "0/0/0/1|0/1/1/1|0/0/1/1" | wc -l` #  it counts all dosages
                        HIGH_het1=`grep HIGH $vcf |cut -f 10-14|grep -E -o "0/0/0/1" | wc -l` #  dosage1
                        HIGH_het2=`grep HIGH $vcf |cut -f 10-14|grep -E -o "0/0/1/1" | wc -l` #  dosage2
                        HIGH_het3=`grep HIGH $vcf |cut -f 10-14|grep -E -o "0/1/1/1" | wc -l` #  dosage3
                        MODIF_het=`grep MODIFIER $vcf |grep -E -v "HIGH|MODERATE|LOW" |cut -f 10-14|grep -E -o "0/0/0/1|0/1/1/1|0/0/1/1" | wc -l`
                        MODIF_het1=`grep MODIFIER $vcf |grep -E -v "HIGH|MODERATE|LOW" |cut -f 10-14|grep -E -o "0/0/0/1" | wc -l`
                        MODIF_het2=`grep MODIFIER $vcf |grep -E -v "HIGH|MODERATE|LOW" |cut -f 10-14|grep -E -o "0/0/1/1" | wc -l`
                        MODIF_het3=`grep MODIFIER $vcf |grep -E -v "HIGH|MODERATE|LOW" |cut -f 10-14|grep -E -o "0/1/1/1" | wc -l`
                        HIGH_hom=`grep HIGH $vcf |cut -f 10-14|grep -E -o "1/1/1/1" | wc -l`
                        MODIF_hom=`grep MODIFIER $vcf |grep -E -v "HIGH|MODERATE|LOW" |cut -f 10-14|grep -E -o "1/1/1/1" | wc -l`
...
echo -e "pop\tploidy\tscf\tnind\tdp_raw\tdp\tsites_tot\tHIGH_tot\tMODERATE_tot\tMODIF_tot\tINTRON_tot\tframeshift_tot\tHIGH_het\tHIGH_hom\tMODIF_het\tMODIF_hom\tHIGH_additive\tMODIF_additive\tminor_sites_tot\tHIGH_tot_minor\tMODIF_tot_minor\tHIGH_tot_singl\tMODIF_tot_singl\tINTRON_tot_minor" > genotype.counts.norm.tsv
...
ls *vcf | parallel  -j $nt "count {}" >> genotype.counts.norm.tsv

mv genotype.counts.norm.tsv $wd/ || export CLEAN_SCRATCH=false # genotype counts goes back to the working directory
...
find $outdir -mindepth 1 -maxdepth 1 -type d -print0 | parallel -0 -j $nt tar -czvf {}.tar.gz {} 

mv $outdir $wd/ || export CLEAN_SCRATCH=false

cd $wd

find $outdir -type f -name '*.tar.gz' -print0 | parallel -0 -j $nt tar -xzvf {}
